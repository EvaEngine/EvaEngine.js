...
test('Custom validator', (t) => {
  const entities = new Entities(`${import.meta.dirname}/../_demo_project/entities`, new Sequelize('database', null, null, { dialect: 'mysql' }));
  t.true(util.isFunction(entities.getInstance().validateIsUnique));
});

test('Multi instances', (t) => {
  const entities1 = new Entities(`${import.meta.dirname}/../_demo_project/entities`, new Sequelize('database', null, null, { dialect: 'mysql' }));
  const entities2 = new Entities(`${import.meta.dirname}/../_demo_project/entities`, new Sequelize('database', null, null, { dialect: 'mysql' }));
